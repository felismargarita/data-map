...
import {PaginationProps,HttpMethodType} from '@/types/commonTypes'
...
const useAntdTable = <T extends {id:number,revision:number}>(tableUrl:string)=>{
  const [pagination,setPagination] = useState<PaginationProps>({current:1,size:10})
  ...
  return {tableProps,selected,onSearch,searchForm}
}
